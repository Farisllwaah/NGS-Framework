...
import org.workflowsim.scheduling.LIBRARYSchedulingAlgorithm;
import org.workflowsim.scheduling.MCTSchedulingAlgorithm;
import org.workflowsim.scheduling.MCTTPCCSchedulingAlgorithm;
...
import org.workflowsim.scheduling.SuperSimpleAlgorithm;
...
public class WorkflowScheduler extends DatacenterBroker {
   ...
   public void processEvent(SimEvent ev) {
    	// System.out.println("The output of the EV"+ev);
        switch (ev.getTag()) {
            ...
            case WorkflowSimTags.LIBRARY_SUBMIT:
                ...
                processLibraryUpdate(ev);
                break;   
            case WorkflowSimTags.VM_STATUS_LIBRARY:
               processLibrarySubmit(ev);
                break;
    ...
    private BaseSchedulingAlgorithm getScheduler(SchedulingAlgorithm name) {
        ...
        switch (name) {
            ...
            case LIBRARY:
			    ...
			    algorithm = new LIBRARYSchedulingAlgorithm();
			    break;
            case MINMIN:
            ...
            case STATIC:
                ...
                break;
            case MCTTPCC:
            	algorithm = new MCTTPCCSchedulingAlgorithm();
           	break;
            case ROUNDROBIN:
                algorithm = new RoundRobinSchedulingAlgorithm();
                break;
            case SUPERSIMPLE:
                algorithm = new SuperSimpleAlgorithm();
                break;
            default:
                algorithm = new StaticSchedulingAlgorithm();
                break;
    ...
    protected void processCloudletUpdate(SimEvent ev) {
      ...
      //  scheduler.setLibraryList(getLibraryList());
 ...
 * @param ev
 */
    protected void processLibraryUpdate(SimEvent ev) {
        ...
        List scheduledList = scheduler.getLibraryScheduledList();
        for (Iterator it = scheduledList.iterator(); it.hasNext();) {
            Library library = (Library) it.next();
            int vmId = library.getVmId();
            double delay = 0.0;
            if(Parameters.getOverheadParams().getQueueDelay()!=null){
                delay = Parameters.getOverheadParams().getQueueDelay(library);
            }
            schedule(getVmsToDatacentersMap().get(vmId), delay, CloudSimTags.LIBRARY_SUBMIT, library);

        }
        getLibraryList().removeAll(scheduledList);
        getCloudletSubmittedList().addAll(scheduledList);
        librarySubmitted += scheduledList.size();
    ...
    private boolean processLibrarySubmitHasShown = false;
    ...
    /**
     * Submits library (task) list
     *
     * @param ev a simEvent object
     */
 
    protected void processLibrarySubmit(SimEvent ev) {
        List<Library> list = (List) ev.getData();
        getCloudletList().addAll(list);
        sendNow(this.getId(), WorkflowSimTags.VM_STATUS_LIBRARY);
        if (!processLibrarySubmitHasShown) {
           //Log.printLine("Pay Attention that the actual vm size is " + getVmsCreatedList().size());
        	processLibrarySubmitHasShown = true;
      }
   }
