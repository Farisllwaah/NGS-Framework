...
public class WorkflowEngine extends SimEntity {
    ...
    protected List<? extends Cloudlet> libraryList;
    private List<Task> libraryofJob;
    /**
     * The job submitted list.
     */
    protected List<? extends Cloudlet> jobsSubmittedList;
    /**
     * The Library submitted list.
     */ 
    protected List<? extends Cloudlet> librarySubmittedList;
    /**
     * The job received list.
     */
    protected List<? extends Cloudlet> jobsReceivedList;
    /**
     * The Library received list.
     */ 
    protected List<? extends Cloudlet> libraryReceivedList;
    /**
     * The job submitted.
     */
    protected int jobsSubmitted;
    /**
     * The Library submitted.
     */
    protected int librarySubmitted;
    ...
    public List<Task> getlibraryofJob() {
        return this.libraryofJob;
    }
    public void addlibraryofJob(Task task) {
        this.libraryofJob.add(task);
    }
...
//        setJobsReceivedList(new ArrayList<Job>());
        ...
        setLibraryList(new ArrayList<Job>());
        setLibrarySubmittedList(new ArrayList<Job>());
        setLibraryReceivedList( new ArrayList<Job>());
        jobsSubmitted = 0;
        librarySubmitted =0;
    ...
    public void processEvent(SimEvent ev) {
        switch (ev.getTag()) {
            ...
            case WorkflowSimTags.JOB_SUBMIT:
                ...
                processJobSubmit(ev);
                break;
            case WorkflowSimTags.LIBRARY_SUBMIT:
               submitLibrarys();
                break;  
    ...
    /**
     * Library a submit event
     *
     * @param ev a SimEvent object
     */
    protected void processLibrarySubmit(SimEvent ev) {
        List<? extends Library> list = (List) ev.getData();
        setLibraryList(list);
    } 
    ...
    protected void processLibraryReturn(SimEvent ev) {

        Library library = (Library) ev.getData();
       
       
        
/*
        getLibraryReceivedList().add(library);
        jobsSubmitted--;
        if (getLibraryList().isEmpty() && librarySubmitted == 0) {
            //send msg to all the schedulers
            for (int i = 0; i < getSchedulerIds().size(); i++) {
                sendNow(getSchedulerId(i), CloudSimTags.END_OF_LIBRARY, null);
            }
        } else {
            sendNow(this.getId(), CloudSimTags.LIBRARY_SUBMIT, null);
        }
...
*/
    ...
    private boolean hasLibraryListContainsID(List libraryList, int id) {
        for (Iterator it = libraryList.iterator(); it.hasNext();) {
            Library library = (Library) it.next();
            if (library.getCloudletId() == id) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Submit Library to the created VMs.
     *
     * @pre $none
     * @post $none
     */
    protected void submitLibrary() {

        List<Library> list = getLibraryList();
        Map allocationList = new HashMap<Integer, List>();
        for (int i = 0; i < getSchedulers().size(); i++) {
            List<Library> submittedList = new ArrayList<Library>();
    ...
    protected void submitJobs() {
    	List submittedarrayofJob = (List) new ArrayList<Task>() ;
        List<Job> list = getJobsList();
       // List<Cloudlet> list1 = getCloudletList();
        int size1 = list.size();
        List<Job> library = getLibraryList();
// Change Here #############################################
 //       Map allocationList = new HashMap<Integer, List>();
        Map <Integer, List> allocationList = new HashMap<>();
        Map allocationlibraryList = new HashMap<Integer, List>();
        for (int i = 0; i < getSchedulers().size(); i++) {
//  Change Here #############################################
//            List<Job> submittedList = new ArrayList<Job>();
         ...
         for (int i = 0; i < getSchedulers().size(); i++) {
            List<Job> submittedlibraryList = new ArrayList<Job>();
            allocationlibraryList.put(getSchedulerId(i), submittedlibraryList);
            
        }	 
        List listLibrary =  new ArrayList<Job>();
 ...
 //                  List submittedList = (List) allocationList.get(job.getUserId());
                   ...
                   /* add array of Library for the selected job: Author Faris*/
                     List<Task> libraryList = job.getChildList();
                 // job.setlibraryofJob(getJobsList());
                    List submittedlibraryList = (List) allocationlibraryList.get(job.getUserId());
                   for (Task child : libraryList) {
                	               	     
                       
                      if (child.getItemType().equals("library")) {
                    	  
                    	
                    	  submittedarrayofJob.add(child);
                    	  submittedlibraryList.add(child);
                    	  librarySubmitted++;
                    	  getLibrarySubmittedList().add(child);
   ...
   /*****************************************************/
     ...
     /*   
        ...
        for (Task child : libraryList) {
             System.out.println("*********************************8"+ child.getItemType().equals("library"));   
             List submittedlibraryList = (List) allocationlibraryList.get(child.getUserId());
            if (child.getItemType().equals("library")) {
            	job.addlibraryofJob(child);
           // 	submittedlibraryList.add(child);
            //	getLibrarySubmittedList().add(child);
            }
          
         //libraryList.remove(child);
         //   i--;
          //  num1--;
   ...
   /******************************************************/     
        ...
        for (int i = 0; i < getSchedulers().size(); i++) {
            ...
            List submittedlibraryList = (List) allocationlibraryList.get(getSchedulerId(i));
            ...
            if (interval > 0 && interval <= size) {
            ...
            }  else if (!submittedList.isEmpty() && (submittedarrayofJob.isEmpty())) {
                ...
                sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT, submittedList);
            	   ...
            	   sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT, submittedlibraryList);
            		submittedlibraryList=null;
             	    sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT, submittedList);
            //	    sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT, submittedList);
            	   
            } 
            	  
            	//	int number1 =submittedList.size();	
            	//	System.out.println("################### The Library Should be submitte   ####################"+number1);
              //   sendNow(this.getSchedulerId(i), CloudSimTags.CLOUDLET_SUBMIT, submittedList);
             //  break;
        }
        }
    ...
    protected void submitLibrarys() {
        ...
        for (int i = 0; i < num; i++) {
            ...
            //Dont use job.isFinished() it is not right
            if (!hasLibraryListContainsID(this.getLibraryReceivedList(), library.getCloudletId())) {
                List<Library> parentList = library.getParentList();
                boolean flag = true;
                for (Library parent : parentList) {
                    if (!hasLibraryListContainsID(this.getLibraryReceivedList(), parent.getCloudletId())) {
                        flag = false;
                        break;
                    }
                ...
                if (flag) {
                    ...
                    List submittedList = (List) allocationList.get(library.getUserId());
                    submittedList.add(library);
                    librarySubmitted++;
                    getLibrarySubmittedList().add(library);
                    list.remove(library);
                    i--;
                    num--;
                }
        ...
        for (int i = 0; i < getSchedulers().size(); i++) {
            ...
            if (interval > 0 && interval <= size) {
                ...
                while (index < size) {
                    ...
                    if (index % interval == 0) {
                        //create a new one
                        schedule(getSchedulerId(i), delay, CloudSimTags.LIBRARY_SUBMIT, subList);
                        delay += delaybase;
                        subList = new ArrayList();
                ...
                sendNow(this.getSchedulerId(i), CloudSimTags.LIBRARY_SUBMIT, submittedList);
            }
    ...
    @SuppressWarnings("unchecked")
    public <T extends Cloudlet> List<T> getLibraryList() {
        return (List<T>) libraryList;
    }
    /**
     * Sets the job list.
     *
    ...
    private <T extends Cloudlet> void setLibraryList(List<T> libraryList) {
   ...
   public <T extends Cloudlet> List<T> getLibrarySubmittedList() {
        return (List<T>) librarySubmittedList;
    }
    /**
    ...
    private <T extends Cloudlet> void setLibrarySubmittedList(List<T> librarySubmittedList) {
        this.librarySubmittedList = librarySubmittedList;
    }
    /**
    ...
    @SuppressWarnings("unchecked")
	public <T extends Cloudlet> List<T> getLibraryReceivedList() {
  
        return (List<T>) libraryReceivedList;
    }
    /**
    ...
    /**
     * Sets the library received list.
     *
     * @param <T> the generic type
     * @param cloudletReceivedList the new library received list
     */
 private <T extends Cloudlet> void setLibraryReceivedList(List<T> libraryReceivedList) {
    	
        this.libraryReceivedList = libraryReceivedList;
    }
    
 /*
    @SuppressWarnings("unused")
	private <L extends Cloudlet> void setLibrarysReceivedList(List<L> libraryReceivedList) {
